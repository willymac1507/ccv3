...
const salonsSelected: Ref<Array<SalonPartial>> = ref([]);
...
function selectSalon(id: number, name: string) {
...
onMounted(async () => {
    ...
    void init();
    //
    // const geocoded = await geocode({ address: 'NR1 2ES' });
        ...
        <div class="flex flex-col space-y-6">
            ...
            <div v-if="mapReady">
                <div class="mb-6 text-sm text-muted-foreground">
                    Select one or more salons by clicking on the map pin. If no
                ...
                >
                    ...
                    <MarkerCluster>
                        <AdvancedMarker
                            ...
                            @click="selectSalon(salon.id, salon.name)"
                        >
                        </AdvancedMarker>
                    </MarkerCluster>
                </GoogleMap>
                <div
                    class="mt-6 flex justify-between text-sm text-muted-foreground"
                >
                    <ul>
                        <li v-for="salon in salonsSelected" :key="salon.id">
                            {{ salon.name }} Selected
                        </li>
                    </ul>
                    <button
                        v-if="salonsSelected.length"
                        class="btn my-auto btn-primary"
                        @click="
                            $inertia.post('/appointments/search', {
                                salons: salonsSelected,
                            })
                        "
                    >
                        Continue
